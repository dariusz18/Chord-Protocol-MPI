...
void build_finger_table() {
    ...
    for (int i = 0; i < M; i++) {
        self.finger[i].start = (self.id + (int)pow(2, i)) % RING_SIZE;
...
int closest_preceding_finger(int key) {
    ...
    for (int i = M - 1; i >= 0; i--) {
        if (in_interval(self.finger[i].node, self.id, key)) {
            if (self.finger[i].node != key) {
                maxi = self.finger[i].node;
                ok = 1;
                break; 
            }
        }
...
void procesare_TAG_LOOKUP_REP(LookupMsg *msg) {
...
void init(LookupMsg *msg, int key) {
...
int main(int argc, char **argv) {
    ...
    while (size_copy > 0) {
        ...
        if (status.MPI_TAG == TAG_LOOKUP_REQ) {
            ...
            procesare_TAG_LOOKUP_REP(msg);
            cnt++;
            if (cnt < nr_lookups) {
                init(msg, lookups[cnt]);
            } else {
                for (int i = 0; i < world_size; i++)
                    MPI_Send(msg, sizeof(LookupMsg), MPI_BYTE, i, TAG_DONE, MPI_COMM_WORLD);
            }
